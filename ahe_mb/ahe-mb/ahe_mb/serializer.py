...
from ahe_sys.models import DeviceType, SiteDeviceList, SiteVariableList
...
from ahe_sys.serializer import SiteVariableSerializer
...
class SiteDeviceSerializer(DetailSerializer):
    device_type = serializers.CharField(
        source='device_type.name', required=False, allow_null=True)

    class Meta:
        model = SiteDevice
        fields = ['id', 'name', 'device_type', 'ip_address',
                  'port', 'unit', 'data_hold_period']
        master_column = "site_device_conf"
        fk_cols = {'device_type': 'name'}
...
class SiteDeviceListSerializer(MasterSerializer):
    ...
    def post_save(self, instance):
        site_var_data = {"site": instance.site.id, "version": 0, "detail": []}
        for site_device in instance.detail.all():
            print("     RG  site deice ver", site_device)
            name_detail = [{"name": x, "device_type":site_device.device_type.name} for x in site_device.get_var_names()]
            site_var_data["detail"].extend(name_detail)
        max_info = SiteVariableList.objects.filter(site=instance.site.id).aggregate(Max('version'))
        if max_info['version__max']:
            site_var_data['version'] = max_info['version__max'] + 1
        svs = SiteVariableSerializer(data=site_var_data)
        if svs.is_valid():
            svs.save()
        else:
            raise ValidationError({"detail": svs.errors})
